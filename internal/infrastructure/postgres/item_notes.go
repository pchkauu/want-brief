...
func (s *Store) GetItemNote(ctx context.Context, id uuid.UUID) (domain.ItemNote, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id, item_id, body, created_at FROM item_notes WHERE id=$1
	`, id).Scan(&n.ID, &n.ItemID, &n.Body, &n.CreatedAt)
	return n, mapErr(err)
}
...
func (s *Store) ListItemNotes(ctx context.Context, itemID uuid.UUID) ([]domain.ItemNote, error) {
	...
	for rows.Next() {
		var n domain.ItemNote
		if err := rows.Scan(&n.ID, &n.ItemID, &n.Body, &n.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, n)
	}
...
func (s *Store) CreateItemNote(ctx context.Context, n domain.ItemNote) (domain.ItemNote, error) {
	err := s.pool.QueryRow(ctx, `
		INSERT INTO item_notes (id, item_id, body, created_at)
		VALUES ($1,$2,$3,$4)
		RETURNING id, item_id, body, created_at
	`, n.ID, n.ItemID, n.Body, n.CreatedAt).
		Scan(&n.ID, &n.ItemID, &n.Body, &n.CreatedAt)
	return n, err
}
